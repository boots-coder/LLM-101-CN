...
```mermaid
flowchart LR
    A(["预训练"]) --> B["SFT/RLHF"] --> C["部署优化"]
    C --> D1["量化"]
    C --> D2["蒸馏"]
    C --> D3["剪枝"]
    C --> D4["推理框架 (vLLM, TensorRT-LLM)"]
    HERE>"★ 你在这里"] -.- D1
```
...
```mermaid
flowchart TD
    S(["矩阵乘法 XW"]) --> D1["1. 检测激活 X 中的离群维度<br/>（|x| > 6.0 的维度）"]
    D1 --> D2["2. 将 X 和 W 按维度拆分"]
    D2 --> O["离群部分 → FP16 计算<br/>（约占 0.1% 的维度）"]
    D2 --> N["正常部分 → INT8 计算<br/>（约占 99.9% 的维度）"]
    O --> ADD["3. 两部分结果相加"]
    N --> ADD
```
